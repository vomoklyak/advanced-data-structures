...
public class AStarShortestPathSearch {

  public static Optional<AStarShortestPathSearchResult> search(
      EdgeWeightedDirectedGraph graph,
      int startVertex,
      int finishVertex,
      Function<DirectedEdge, Double> heuristic) {
    validateGraph(graph);
    ...
    var edges = new EdgeWeightedDirectedGraph.DirectedEdge[graph.getVertexNum()];
      ...
      if (fromVertex == finishVertex) {
        ...
        graph.adjacentEdges(fromVertex).forEach(adjacentEdge -> {
          ...
          if (distances[toVertex] > distances[fromVertex] + adjacentEdge.getDistance()) {
            ...
            var meteredVertex = new MeteredVertex(
                toVertex, distances[toVertex] + heuristic.apply(adjacentEdge));
  ...
  private static AStarShortestPathSearchResult path(
    ...
    return new AStarShortestPathSearchResult(vertices, distance, numberOfVisitedEdges);
  }
  ...
  public record AStarShortestPathSearchResult(
